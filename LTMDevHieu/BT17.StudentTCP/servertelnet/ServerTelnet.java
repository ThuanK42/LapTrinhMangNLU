...
public class ServerTelnet {
	...
	public static void main(String[] args) throws IOException {
		...
		while((text = br.readLine()) != null) {
			...
			if (index > 0) cmd = text.substring(0, index).trim();
			...
			} else if ((cmd.compareToIgnoreCase("exit") == 0) || (cmd.compareToIgnoreCase("quit") == 0)){
				pw.println("Goodbye!");
				server.close();
				break;
			} else {
	...
	public static String findByName(List<Student> list, String name) throws IOException {
		System.out.println("findByName:[" + name + "]");
		String text = "";
		for (Student s : list) {
			System.out.println("check:" + s.getName() + ".constains=" + s.getName().contains(name));
	...
	public static String findAgeUnder(List<Student> list, int age) throws IOException {
		String text = "";
		for (Student s : list) {
			if (s.getAge() <= age)
				text += s.getSid() + "." + s.getName() + ", " +  s.getAge() + "\n\r";
	...
	public static String findAgeUpper(List<Student> list, int age) throws IOException {
		String text = "";
		for (Student s : list) {
			if (s.getAge() >= age)
				text += s.getSid() + "." + s.getName() + ", " +  s.getAge() + "\n\r";
